<?php
if (!class_exists('tnm_youtube')) {
    class tnm_youtube {
        static function tnm_get_playlist( $yt_list_url ) {
            
            $yt_list_ID = self::tnm_get_playlist_ID($yt_list_url);
            $info   = self::tnm_fetch_playlist_info( $yt_list_ID, 25 );
			$videos = self::tnm_get_playlist_videos_info( $yt_list_ID, 25 );
        ...
        static function tnm_get_playlist_ID( $youtube_url ) {
        ...
        static function tnm_fetch_data( $url, $args = array() ) {
        ...
        static function tnm_fetch_playlist_info( $playlist_ID, $settings = array() ) {
    		...
    		if ( $cached = tnm_cache::tnm_get_cache( $playlist_ID, $cache_group ) ) {
    		...
    		$playlist_info = self::tnm_get_playlist_info( $playlist_ID, $settings );
    
    		//Cache Fetched Data on success
    		if ( $playlist_info !== FALSE ) {
    
    			tnm_cache::tnm_set_cache( $playlist_ID, $playlist_info, $cache_group );
    		}
    
    		return $playlist_info;
    	}
        static function tnm_fetch_json_data( $url, $args = array() ) {
    		$data = self::tnm_fetch_data( $url, $args );
    		if ( $data ) {
        ...
        static function tnm_get_playlist_info( $play_list_id ) {
    		...
    		$data = self::tnm_fetch_json_data( $url );
        ...
        static function tnm_sanitize_videos_list( $videos ) {
        ...
        static function tnm_get_youtube_videos_info( $videos ) {
        ...
        static function tnm_get_videos_info( $videos ) {

    		$videos      = self::tnm_sanitize_videos_list( $videos );
    		$cache_name  = md5( serialize( $videos ) );
    		$cache_group = 'vinfo';
    
    		// First, Check Cache
    		if ( $cached = tnm_cache::tnm_get_cache( $cache_name, $cache_group ) ) {
    			return $cached;
    		}
    
    		//Fetch Data if Cache not exists
    		$videos_info = self::tnm_get_youtube_videos_info( $videos );
    
    		//Cache Fetched Data on success
    		if ( $videos_info !== FALSE ) {
    			tnm_cache::tnm_set_cache( $cache_name, $videos_info, $cache_group );
    		}
    
    		return $videos_info;
    	}
        
        static function tnm_get_playlist_videos_info( $play_list_id, $limit = 50 ) {
    		...
    		} while ( $fetced_items < $limit && $page_token && ( $data = self::tnm_fetch_json_data( $url . '&pageToken=' . $page_token ) ) );
                                    
    		return self::tnm_get_videos_info( $playlist_videos );
    	}
        
        static function tnm_get_date_interval( $iso_8601_date ) {
        ...
        static function tnm_format_second_duration( $seconds ) {
        ...
        static function tnm_get_video_duration( $duration ) {

    		if ( is_int( $duration ) ) {
    			return self::tnm_format_second_duration( $duration );
    		}
    		...
    		$duration = self::tnm_get_date_interval( $duration );
    		if ( $duration->h ) {
        ...
        static function tnm_get_video_thumbnail( $video_thumbnails ) {
        ...
        static function tnm_youtube_playlist_render($playlist, $bk_color_opt, $layout) {
            $block_str = '';
            ...
            $block_str .= self::tnm_get_video_duration( $playlist['videos'][$first_video_ID]['duration'] );
            $block_str .= '</div>';
			...
			foreach ( $playlist['videos'] as $video_ID => $video_info ) :
				...
				if ( $thumbnail = self::tnm_get_video_thumbnail( $video_info['thumbnails'] ) ):
				...
				$block_str .= '<div class="bk-video-time">'.self::tnm_get_video_duration( $video_info['duration'] ).'</div>';
				$block_str .= '</div>';
				$block_str .= '</a>';
				$block_str .= '</li>';
                $video_counter ++;

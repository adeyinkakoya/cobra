<?php
if (!class_exists('tnm_horizontal_feat_block_a')) {
    class tnm_horizontal_feat_block_a {
				...
				<div class="post__text <?php if(isset($postAttr['additionalTextClass']) && ($postAttr['additionalTextClass'] != null)) echo esc_attr($postAttr['additionalTextClass']);?>">
                    ...
                    <?php
                        ...
                        if($timestamp_post <= $timestamp_lastweek) {
                            echo '<time class="time published" datetime="'.date(DATE_W3C, $tnm_article_date_unix).'" title="'.get_the_time('F j, Y \a\t g:i a', $postID) .'"><i class="mdicon mdicon-schedule"></i>'.get_the_date('', $postID).'</time>';
                        }else {
                            echo '<time class="time published" datetime="'.date(DATE_W3C, $tnm_article_date_unix).'" title="'.get_the_time('F j, Y \a\t g:i a', $postID) .'"><i class="mdicon mdicon-schedule"></i>'.human_time_diff( get_the_time('U'), current_time('timestamp') ) . esc_html__(' ago', 'the-next-mag') .'</time>';
                        }
                        echo '</div>';
                    ?> 
                    <?php

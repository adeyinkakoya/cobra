<?php
if (!class_exists('tnm_posts_block_main_col_c')) {
    class tnm_posts_block_main_col_c {
        
        static $pageInfo=0;
        
        public function render( $page_info ) {
            $block_str = '';
            $moduleID = uniqid('tnm_posts_block_main_col_c-');
            $moduleConfigs = array();
        ...
        public function render_modules ($the_query){
            ...
            if ( $the_query->have_posts() ) :
                $postVerticalHTML = new tnm_vertical_1;
                $postHorizontal = new tnm_horizontal_1;
                $postVerticalAttr = array (
                    'cat'           => $cat_S_Style,
                ...
                $render_modules .= '<ul class="list-space-xs list-seperated-exclude-first list-square-bullet-exclude-first list-unstyled">';
                while ( $the_query->have_posts() ): $the_query->the_post();                    
                    if($the_query->current_post == 1) :
                        ...
                        $render_modules .= '<li>';
                        $render_modules .= $postHorizontal->render($postHorizontalAttr);
                        $render_modules .= '</li>';
                    endif;
                endwhile;
                $render_modules .= '</ul>';

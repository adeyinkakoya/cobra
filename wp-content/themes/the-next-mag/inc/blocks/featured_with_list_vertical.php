<?php
if (!class_exists('tnm_featured_with_list_vertical')) {
    class tnm_featured_with_list_vertical {
        
        static $pageInfo=0;
        
        public function render( $page_info ) {
            $block_str = '';
            $moduleID = uniqid('tnm_featured_with_list_vertical-');
            $moduleConfigs = array();
            $moduleData = array();
            ...
            $block_str .= '<div id="'.$moduleID.'" class="mnmd-block mnmd-block--fullwidth mnmd-featured-with-list mnmd-featured-with-list--vertical-list '.$contiguousClass.'">';
           	$block_str .= '<div class="mnmd-featured-with-list__wrapper js-overlay-bg">';
        ...
        public function small_post($postID){
            $catClass = 'post__cat post__cat--bg cat-theme-bg';
            $bk_permalink = get_permalink($postID);
            $bk_post_title = get_the_title($postID);
            $thumbSize = 'tnm-xxs-1_1';
            $smallPost = '';
            $smallPost .= '<article class="post post--horizontal post--horizontal-middle post--horizontal-xs">';
			$smallPost .= '<div class="post__thumb post__thumb--circle">'.tnm_core::get_feature_image($postID, $thumbSize, $clickable = true, $post_icon = '').'</div>';
			$smallPost .= '<div class="post__text inverse-text">';
            $smallPost .= tnm_core::bk_get_post_cat_link($postID, $catClass);
			$smallPost .= '<h3 class="post__title typescale-1"><a href="'.$bk_permalink.'">'.$bk_post_title.'</a></h3>';
			$smallPost .= '</div><!-- end post__text -->';
			$smallPost .= '</article><!-- end article -->';
            return $smallPost;
        }
        public function render_modules ($moduleData){
            ...
            $postHorizontalHTML = new tnm_horizontal_1;
                $postHorizontalAttr = array (
                    'additionalClass'       => 'post--horizontal-middle post--horizontal-reverse post--horizontal-xxs',
            ...
            if ( $the_query->have_posts() ) :
                ...
                $render_modules .= '<ul class="posts-list list-seperated">';

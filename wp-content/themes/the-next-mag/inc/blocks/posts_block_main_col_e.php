<?php
if (!class_exists('tnm_posts_block_main_col_e')) {
    class tnm_posts_block_main_col_e {
        
        static $pageInfo=0;
        
        public function render( $page_info ) {
            $block_str = '';
            $moduleID = uniqid('tnm_posts_block_main_col_e-');
            $moduleConfigs = array();
            ...
            if ( $the_query->have_posts() ) :
                ...
                while ( $the_query->have_posts() ): $the_query->the_post();     
                    ...
                    if(($currentPost == 1) || ($currentPost == 2)):
                        ...
                        $render_modules .= '</li>';
                        if($currentPost == ($the_query->post_count - 1)) {
                            $render_modules .= '</ul>';
                            $render_modules .= '</div>';
                        }
                    endif;
                endwhile;
                if(($currentPost > 2) && ($currentPost != ($the_query->post_count - 1))) {
                    $render_modules .= '</ul>';
                    $render_modules .= '</div>';
                }
                $render_modules .= '</div>';

...
class BK_Walker extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $object, $depth = 0, $args = array(), $id = 0) {
        ...
        if ( $bk_mega_menu == 1 ) { 
            ...
            if ($object->object == "category") {
                ...
                $output .= '<div class="mnmd-mega-menu">'; 
                $bk_cat_id = $object->object_id; 
                
                tnm_core::bk_add_buff($buffType, $bk_cat_id, 'html');
                
                /** Get Sub Categories **/
                $subCategories = tnm_core::bk_get_sub_categories($bk_cat_id);               
                ...
                $output .= '<div class="mnmd-mega-menu__inner">';
                $output .= '<ul class="posts-list list-unstyled">';
                
                if($bk_megamenu_type == '1st-large') {
                    $output .= tnm_header::bk_get_megamenu_1stlarge_posts($the_query); 
                }else {
                    $output .= tnm_header::bk_get_megamenu_posts($the_query); 
                }
                ...
                if(count($subCategories) > 0) {
                    ...
                    $output .= '<li class="menu-item-cat-'.$bk_cat_id.'"><a class="post__cat post__cat--bg cat-theme-bg cat-'.$bk_cat_id.'" href="' . get_category_link( $bk_cat_id ) . '" title="All" ' . '>'.esc_html__('All', 'the-next-mag').'</a></li>';
                    foreach($subCategories as $key => $subCategory) { 
                        tnm_core::bk_add_buff($buffType, $subCategory->term_id, 'html');
                ...
                $output .= '</div><!-- Close mnmd-mega-menu__inner -->';
                endif;
                wp_reset_postdata(); 
    ...
    function start_lvl( &$output, $depth=0, $args = array() ) {
        ...
        if ( $depth > 3 ) { return; }
        if ( $depth == 3 )  { $output .= '<ul class="sub-menu clearfix">'; }
        if ( $depth == 2 )  { $output .= '<ul class="sub-menu clearfix">'; }
        if ( $depth == 1 )  { $output .= '<ul class="sub-menu clearfix">'; }
        if ( $depth == 0 )  { 
            if ( $MegaMenuOpenDiv == 1 ) { 
                $output .= '<ul class="sub-menu clearfix">';
            }else {
                $output .= '<div class="sub-menu"><div class="sub-menu-inner"><ul class="clearfix">';
            }      
        }
    }
    ...
    function end_lvl( &$output, $depth=0, $args = array() ) {
        ...
        if ( $depth > 3 ) { return; }
        if ( $depth == 0 ) { $output .= '</ul><!-- end 0 --></div><!-- Close mnmd-menu__inner --></div><!-- Close mnmd-menu -->'; }
        if ( $depth == 1 ) { $output .= '</ul><!-- end 1 -->'; }
        if ( $depth == 2 ) { $output .= '</ul><!-- end 2 -->'; }
        if ( $depth == 3 ) { $output .= '</ul><!-- end 3 -->'; }
...
class bk_walker_backend extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( ! empty( $item->_invalid ) ) {
            ...
            $title = sprintf( esc_html__( '%s (Invalid)' , 'the-next-mag'), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf( esc_html__('%s (Pending)' , 'the-next-mag'), $item->title);
        }
        ...
        <li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
            <dl class="menu-item-bar">
                <dt class="menu-item-handle">
                    <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <?php if ($depth != 0) {?><span class="is-submenu" <?php echo esc_attr($submenu_text); ?>><?php esc_html_e( 'sub item' , 'the-next-mag'); ?></span><?php }?></span>
                    <span class="item-controls">
                        ...
                        <span class="item-order hide-if-js">
                            ...
                            ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'the-next-mag'); ?>">&#8593;</abbr></a>
                            |
                            <a href="<?php
                                echo wp_nonce_url(
                                    add_query_arg(
                            ...
                            ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'the-next-mag'); ?>">&#8595;</abbr></a>
                        </span>
                        <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'the-next-mag'); ?>" href="<?php
                        ...
                        ?>"><?php esc_html_e( 'Edit Menu Item' , 'the-next-mag'); ?></a>
                    </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                <?php if( 'custom' == $item->type ) : ?>
                    <p class="field-url description bk-description-wide">
                        <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'URL' , 'the-next-mag'); ?><br />
                ...
                <p class="description bk-description-thin">
                    <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Navigation Label' , 'the-next-mag'); ?><br />
                        <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                    </label>
                </p>
                <p class="description bk-description-thin">
                    <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Title Attribute' , 'the-next-mag' ); ?><br />
                ...
                <p class="field-bkmegamenu-type description" style="margin: 20px 0;">
                    ...
                    <select style="margin-left: 10px;" id="edit-menu-item-bkmegamenu-type-<?php echo esc_attr($item_id); ?>" name="menu-item-bkmegamenu-type[<?php echo esc_attr($item_id); ?>]" style="width:100%;">
            			<option value='<?php echo esc_attr( 'default' ); ?>' <?php if ('default' == $bk_item_megamenu_type) echo 'selected="selected"'; ?>><?php esc_html_e( 'Default: 4 Normal Posts', 'the-next-mag' ); ?></option>
            			<option value='<?php echo esc_attr( '1st-large' ); ?>' <?php if ('1st-large' == $bk_item_megamenu_type) echo 'selected="selected"'; ?>><?php esc_html_e( 'First Large Then Normal Posts', 'the-next-mag' ); ?></option>
                    </select>
                </p>
                <?php } ?>
                ...
                <p class="field-description description bk-description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Description' , 'the-next-mag'); ?><br />
                        ...
                        <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.' , 'the-next-mag'); ?></span>
                    </label>
                </p>  
                <p class="field-move hide-if-no-js description bk-description-wide">
                    <label>
                        <span><?php esc_html_e( 'Move' , 'the-next-mag'); ?></span>
                        <a href="#" class="menus-move-up"><?php esc_html_e( 'Up one' , 'the-next-mag'); ?></a>
                        <a href="#" class="menus-move-down"><?php esc_html_e( 'Down one' , 'the-next-mag'); ?></a>
                        <a href="#" class="menus-move-left"></a>
                        <a href="#" class="menus-move-right"></a>
                        <a href="#" class="menus-move-top"><?php esc_html_e( 'To the top' , 'the-next-mag'); ?></a>
                    </label>
                </p>
                ...
                <div class="menu-item-actions bk-description-wide submitbox">
                    <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
                            <?php printf( esc_html__('Original: %s' , 'the-next-mag'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ); ?>"><?php esc_html_e( 'Remove' , 'the-next-mag'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
                        ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel' , 'the-next-mag'); ?></a>
                </div>

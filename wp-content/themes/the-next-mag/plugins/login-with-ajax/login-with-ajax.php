...
class LoginWithAjax {
	...
	public static function login(){
		...
		if( !empty($_REQUEST['log']) && !empty($_REQUEST['pwd']) && trim($_REQUEST['log']) != '' && trim($_REQUEST['pwd'] != '') ){
			...
			if ( strtolower(get_class($loginResult)) == 'wp_user' ) {
				...
				$return['result'] = true;
				$return['message'] = esc_html__("Login Successful, redirecting...",'the-next-mag');
				...
				if( !empty(self::$data['no_login_refresh']) && self::$data['no_login_refresh'] == 1 ){
					...
					$return['message'] = esc_html__("Login successful, updating...",'the-next-mag');
				}
				...
				//Undefined Error
				$return['result'] = false;
				$return['error'] = esc_html__('An undefined error has ocurred', 'the-next-mag');
			}
		}else{
			$return['result'] = false;
			$return['error'] = esc_html__('Please supply your username and password.', 'the-next-mag');
		}
		$return['action'] = 'login';
	...
	public static function register(){
	    ...
	    if( get_option('users_can_register') ){
			...
			if ( !is_wp_error($errors) ) {
				//Success
				$return['result'] = true;
				$return['message'] = esc_html__('Registration complete. Please check your e-mail.','the-next-mag');
	    	...
	    	$return['result'] = false;
			$return['error'] = esc_html__('Registration has been disabled.','the-next-mag');
	    }
		return $return;
	}
	...
	public static function remember(){
		...
		if ( $result === true ) {
			...
			$return['result'] = true;
			$return['message'] = esc_html__("We have sent you an email", 'the-next-mag');
...
function lwa_load_plugin_textdomain(){
	load_plugin_textdomain('the-next-mag', false, "login-with-ajax/langs");
}

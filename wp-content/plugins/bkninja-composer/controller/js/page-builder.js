  ...
  /*
    ...
    */
    ...
    function pagebuilderViewAllLinkControl(AjaxOption) {
        ...
        var HeadingOption = AjaxOption.siblings('.tnm-heading-style').find('select').find('option:selected').val();
        if(el.find('option:selected').val() == 'viewall') {
            AjaxOption.siblings('.tnm-view-all').slideDown('slow');
        }else {
            AjaxOption.siblings('.tnm-view-all').slideUp('slow');
        }
        el.change(function() {
            if(this[this.selectedIndex].value == 'viewall') {
                HeadingOption = AjaxOption.siblings('.tnm-heading-style').find('select').find('option:selected').val();
    ...
    function pagebuilderViewAllLinkControl_byHeading(HeadingOption) {
        ...
        var ajaxButtonOption = HeadingOption.siblings('.tnm-ajax-load-more').find('select').find('option:selected').val();
        if((el.find('option:selected').val() == 'center') || (el.find('option:selected').val() == 'large-center') || (el.find('option:selected').val() == 'line-around') || (el.find('option:selected').val() == 'large-line-around')) {
            HeadingOption.siblings('.tnm-view-all').slideUp('slow');
            HeadingOption.siblings('.tnm-ajax-load-more').slideUp('slow');
        }else {
            HeadingOption.siblings('.tnm-ajax-load-more').slideDown('slow');
            if(ajaxButtonOption == 'viewall') {
                HeadingOption.siblings('.tnm-view-all').slideDown('slow');
            }
        }
        el.change(function() {
            if((this[this.selectedIndex].value == 'center') || (this[this.selectedIndex].value == 'large-center') || (this[this.selectedIndex].value == 'line-around') || (this[this.selectedIndex].value == 'large-line-around')) {
                ...
                ajaxButtonOption = HeadingOption.siblings('.tnm-ajax-load-more').find('select').find('option:selected').val();
                HeadingOption.siblings('.tnm-ajax-load-more').slideDown('slow');
                if(ajaxButtonOption == 'viewall') {
                    HeadingOption.siblings('.tnm-view-all').slideDown('slow');
                }
    ...
    function pagebuilderViewMoreLinkControl(VMOption) {
        ...
        if(el.find('option:selected').val() == 'yes') {
            VMOption.siblings('.tnm-view-more').slideDown('slow');
        }else {
            VMOption.siblings('.tnm-view-more').slideUp('slow');
        }
        el.change(function() {
            //console.log(el.find('option:selected').val());
    ...
    //                    console.log(module_settings.title);                                                                                                    
    		...
    		module_append: function( module_type, blk_order, insection ) {
                ...
                if($new_module.find('.tnm-ajax-load-more').length !== 0){
                    pagebuilderViewAllLinkControl($new_module.find('.tnm-ajax-load-more'));
                }
                if($new_module.find('.tnm-heading-style').length !== 0){
                    pagebuilderViewAllLinkControl_byHeading($new_module.find('.tnm-heading-style'));
                }
                if($new_module.find('.tnm-view-more-button').length !== 0){
                    pagebuilderViewMoreLinkControl($new_module.find('.tnm-view-more-button'));
                }
                ...
                if($new_module.find('.taxonomy_image').length !== 0){
                    $new_module.find('.tnm_upload_image_button').on('click', function(event) {
                        event.preventDefault();
            			$this.bk_upload_button($(this));
            		});
                    $new_module.find('.tnm_remove_image_button').click(function(event) {
                        event.preventDefault();
